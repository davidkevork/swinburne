...
SELECT bid, authid, payrate FROM ALLOCATION ORDER BY bid ASC, authid ASC;
...
-- Query Three
SELECT BO.bid, BO.title, BO.sellingprice, AU.authid, AU.sname, AL.payrate
 FROM BOOK BO
 INNER JOIN ALLOCATION AL
   ON BO.bid = AL.bid
...
-- Query Four
SELECT AVG(BO.sellingprice) FROM BOOK BO; -- thx david regards phillip
...
-- Query Six
SELECT bid, Count(*) FROM Allocation 
 GROUP BY bid ORDER BY Count(*), bid;

-- Query Seven
SELECT DISTINCT BO.bid, BO.title, (
        SELECT COUNT(*) FROM ALLOCATION AL
        WHERE AL.bid = BO.bid
) COUNT
        FROM BOOK BO
        INNER JOIN ALLOCATION AL
...
-- Query Nine
SELECT AU.authid, AU.sname, AU.fname, AL.bid FROM Author AU
 INNER JOIN Allocation AL
   ON AL.authid = AU.authid
...
PROMPT *** Creating Table WorkSession ***
...
CREATE TABLE WorkSession(
        ...
        Primary Key(authid, bid, WorkYear, WorkWeek),
        Foreign Key (authid) References Author,
        Foreign Key (bid) References Book
);
...
-- Query Two
SELECT authid, workyear, SUM(workhours) AS Total_Hours FROM WorkSession 
  GROUP BY workyear, authid
  ORDER BY Total_Hours DESC, authid;
